...
document.addEventListener("DOMContentLoaded", () => {
  ...
  askButton.addEventListener("click", async () => {
    ...
    if (!resultData) {
      try {
        const raw = localStorage.getItem("floraScanDataset");
        const dataset = raw ? JSON.parse(raw) : null;
        if (Array.isArray(dataset) && dataset.length > 0) {
          const p = dataset[0];
          resultData = {
            name: p.name || "Identified Plant",
            ...
            characteristics: Array.isArray(p.characteristics) ? p.characteristics : ["Green foliage", "Adaptable"],
            care: p.care || "",
            image: hasPhoto ? previewImage.src : (p.imageUrl || "")
          };
        }
      } catch (_) {}
    }
  ...
  // Common Plants grid from dataset
  ...
  if (plantGrid) {
    try {
      ...
      const dataset = raw ? JSON.parse(raw) : null;
      if (Array.isArray(dataset) && dataset.length > 0) {
        plantGrid.innerHTML = dataset.map((p) => {
          const img = (p.imageUrl || "").trim();
          const style = img ? "background-image: url(" + img.replace(/"/g, "&quot;") + "); background-size: cover; background-position: center;" : "";
          return (
            '<article class="plant-card">' +
            '<div class="plant-photo" style="' + style + '"></div>' +
            '<div class="plant-info">' +
            '<div class="plant-name">' + (p.name || "—").replace(/</g, "&lt;") + "</div>" +
            '<div class="plant-scientific">' + (p.scientificName || "").replace(/</g, "&lt;") + "</div>" +
            "</div></article>"
          );
        }).join("");
      }
